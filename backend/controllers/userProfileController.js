...
const updateUserProfile = async (req, res) => {
  try {
    const { fullName, username, phone, dob, country, bio, isPrivate } = req.body;
    ...
    if (req.user.followers) {
      req.user.followers = req.user.followers.filter(id => 
        id && typeof id === 'object' && id.toString && id.toString().length === 24
      );
    }
    ...
    if (req.user.followRequests) {
      req.user.followRequests = req.user.followRequests.filter(id => 
        id && typeof id === 'object' && id.toString && id.toString().length === 24
      );
    }
    ...
    res.status(200).json({
      success: true,
      message: isPrivate !== undefined ? "Privacy settings updated successfully" : "Profile updated successfully",
      user: req.user
    });
  } catch (error) {
